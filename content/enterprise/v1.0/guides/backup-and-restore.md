...
Currently, users can perform backups and restores for all data in the cluster, or for one database and/or retention policy, or for a single shard.

## Terminology and behavior

A **backup** creates a copy of the cluster’s data and metadata at that point in time and stores the copy in the specified directory.
...
By default, a restore attempts to write databases with the same replication factor as when the backup was retrieved.
...
* `-bind <hostname>:8091`: hostname and HTTP port of a running meta server (defaults to `localhost:8091`)
* `-db <name>`: name of single database to back up
* `-rp string`: name of single retention policy to back up (must specify `-db` with `-rp`)
* `-shard uint`: ID of a single shard to back up
...
Point at a remote meta server and back up only one database into a given directory (directory must already exist):
```
...
##### Restoring a backed up database and merging it into an existing database

Imagine that your telegraf database was mistakenly dropped, but you have a recent backup so you've only lost a small amount of data.
...
Then, in the `influx` client, use `SELECT INTO` to copy the raw data from the new database into the existing `telegraf` database:
